...
// ! i dont think we are allowed to use MPI_Sendrecv. Instead we can use MPI_Isend and MPI_Irecv with MPI_Waitall to achieve the same effect. I will update the code to reflect this change.
typedef struct {
...
int main(int argc, char *argv[]) {
    ...
    for (int t = 0; t < T; t++) {
        for (int i = 0; i < F; i++) {
            ...
            local_counts[i] = d_point_stencil(data[i], next[i], nx, ny, nz, d, isovalue, &halo);
        }
    ...
    if (rank == 0) {
        for (int i = 0; i < F; i++) {
            for (int t = 0; t < T; t++) {
                printf("field=%d timestep=%d isovalue_count=%lld\n",
                       i + 1, t + 1, global_counts[(size_t)t * F + i]);
            }
        }
        printf("max_time=%f\n", max_time);
    }
